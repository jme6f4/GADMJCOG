...
# # D) readModels/summaries ####
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
ReadModels14 <- readModels(target = paste0(getwd(),"/Work/Models/",Model),
                           filefilter = date)
library(plyr)
Summary14 <- as_tibble(do.call("rbind.fill", sapply(ReadModels14, "[", "summaries")))
detach("package:plyr", unload=TRUE)
Summary14_Problem <- Summary14 %>% dplyr::filter(is.na(AIC))
View(Summary14_Problem)
# 
# 
# 
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
...
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
unstandardized14 <- sapply(sapply(ReadModels14, 
                                  "[", "parameters"), 
                           "[", "unstandardized")
lapply(names(unstandardized14), function(element) {
  unstandardized14[[element]]$filename <<- element
  })

stdyx.standardized14 <- sapply(sapply(ReadModels14, 
                                  "[", "parameters"), 
                           "[", "stdyx.standardized")

lapply(names(stdyx.standardized14), function(element) {
  stdyx.standardized14[[element]]$filename <<- element
  })
...
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
Estimates_14_Unst <- as_tibble(do.call("rbind", unstandardized14))  %>%
...
Estimates_14_Cog_WI_Unst <- Estimates_14_Unst %>%
  filter(grepl("Cog", Column)) %>%
  filter(grepl(")",Column)) %>%
  filter(grepl("WI",Column)) 
Estimates_14_Cog_BW_Unst <- Estimates_14_Unst %>%
  filter(grepl("Cog", Column)) %>%
  filter(grepl("",Column)) %>%
  filter(grepl("BW",Column)) 

Estimates_14_Can_WI_Unst <- Estimates_14_Unst %>%
  filter(grepl("Mj", Column)) %>%
  filter(grepl(")",Column)) %>%
  filter(grepl("WI",Column)) 
Estimates_14_Can_BW_Unst <- Estimates_14_Unst %>%
  filter(grepl("Mj", Column)) %>%
  filter(grepl("",Column)) %>%
  filter(grepl("BW",Column)) 

Estimates_14_Alc_WI_Unst <- Estimates_14_Unst %>%
  filter(grepl("Alc", Column)) %>%
  filter(grepl(")",Column)) %>%
  filter(grepl("WI",Column)) 
Estimates_14_Alc_BW_Unst <- Estimates_14_Unst %>%
  filter(grepl("Alc", Column)) %>%
  filter(grepl(")",Column))%>%
  filter(grepl("BW",Column))  

hist(Estimates_14_Cog_WI_Unst$pval)
hist(Estimates_14_Cog_BW_Unst$pval)
hist(Estimates_14_Can_WI_Unst$pval)
hist(Estimates_14_Can_BW_Unst$pval)
hist(Estimates_14_Alc_WI_Unst$pval)
hist(Estimates_14_Alc_BW_Unst$pval)

Estimates_14_Tab_Unst  <- Estimates_14_Unst %>%
  dplyr::select(filename, Column, Row, 
         DisplayFull, pval) %>%
...
TableEstimates_14_Unst <- Estimates_14_Tab_Unst %>%
  filter(grepl(")",Column)) %>%
...
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
Estimates_14_Stdyx <- as_tibble(do.call("rbind", stdyx.standardized14))  %>%
  mutate(Column = ifelse(param == "COGPGS", "2a) WI Cog PGS",
                  ifelse(param == "COGPGSBW", "1a) BW Cog PGS",
...
Estimates_14_Cog_WI_Stdyx <- Estimates_14_Stdyx %>%
  filter(grepl("Cog", Column)) %>%
  filter(grepl(")",Column)) %>%
  filter(grepl("WI",Column)) 
Estimates_14_Cog_BW_Stdyx <- Estimates_14_Stdyx %>%
  filter(grepl("Cog", Column)) %>%
  filter(grepl("",Column)) %>%
  filter(grepl("BW",Column)) 
Estimates_14_Can_WI_Stdyx <- Estimates_14_Stdyx %>%
  filter(grepl("Mj", Column)) %>%
  filter(grepl(")",Column)) %>%
  filter(grepl("WI",Column)) 
Estimates_14_Can_BW_Stdyx <- Estimates_14_Stdyx %>%
  filter(grepl("Mj", Column)) %>%
  filter(grepl("",Column)) %>%
  filter(grepl("BW",Column)) 
Estimates_14_Alc_WI_Stdyx <- Estimates_14_Stdyx %>%
  filter(grepl("Alc", Column)) %>%
  filter(grepl(")",Column)) %>%
  filter(grepl("WI",Column)) 
Estimates_14_Alc_BW_Stdyx <- Estimates_14_Stdyx %>%
  filter(grepl("Alc", Column)) %>%
  filter(grepl(")",Column))%>%
  filter(grepl("BW",Column))  

hist(Estimates_14_Cog_WI_Stdyx$pval)
hist(Estimates_14_Cog_BW_Stdyx$pval)
hist(Estimates_14_Can_WI_Stdyx$pval)
hist(Estimates_14_Can_BW_Stdyx$pval)
hist(Estimates_14_Alc_WI_Stdyx$pval)
hist(Estimates_14_Alc_BW_Stdyx$pval)

Estimates_14_Tab_Stdyx  <- Estimates_14_Stdyx %>%
  dplyr::select(filename, Column, Row, 
         DisplayFull, pval) %>%
  mutate(EstP = as.numeric(pval)) %>%
  group_by(Row, Column) %>%
...
TableEstimates_14_Stdyx <- Estimates_14_Tab_Stdyx %>%
  filter(grepl(")",Column)) %>%
  spread(key = Column, value = DisplayFull) 
...
write_csv(x = TableEstimates_14_Unst,
          na = "",
          path = paste0(getwd(), "/Work/Results/", Model, "_Unst_Filt_New2_", date,".csv"))

write_csv(x = TableEstimates_14_Stdyx,
          na = "",
