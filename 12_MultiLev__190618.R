...
# # D) readModels/summaries ####
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
ReadModels12 <- readModels(target = paste0(getwd(),"/Work/Models/",Model),
                           filefilter = c("_1_19|_2_19"))
library(plyr)
Summary12 <- as_tibble(do.call("rbind.fill", sapply(ReadModels12, "[", "summaries")))
detach("package:plyr", unload=TRUE)
Summary12_Problem <- Summary12 %>% dplyr::filter(is.na(AIC))
View(Summary12_Problem)
...
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
unstandardized12 <- sapply(sapply(ReadModels12, 
                                  "[", "parameters"), 
                           "[", "unstandardized")
lapply(names(unstandardized12), function(element) {
  unstandardized12[[element]]$filename <<- element
  })

stdyx.standardized12 <- sapply(sapply(ReadModels12, 
                                  "[", "parameters"), 
                           "[", "stdyx.standardized")

lapply(names(stdyx.standardized12), function(element) {
  stdyx.standardized12[[element]]$filename <<- element
  })
...
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
Estimates_12_Unst <- as_tibble(do.call("rbind", unstandardized12))  %>%
...
Estimates_12_Cog_WI_Unst <- Estimates_12_Unst %>%
  filter(grepl("Cog", Column)) %>%
  filter(grepl(")",Column)) %>%
  filter(grepl("WI",Column)) 
Estimates_12_Cog_BW_Unst <- Estimates_12_Unst %>%
  filter(grepl("Cog", Column)) %>%
  filter(grepl("",Column)) %>%
  filter(grepl("BW",Column)) 

Estimates_12_Can_WI_Unst <- Estimates_12_Unst %>%
  filter(grepl("Mj", Column)) %>%
  filter(grepl(")",Column)) %>%
  filter(grepl("WI",Column)) 
Estimates_12_Can_BW_Unst <- Estimates_12_Unst %>%
  filter(grepl("Mj", Column)) %>%
  filter(grepl("",Column)) %>%
  filter(grepl("BW",Column)) 

Estimates_12_Alc_WI_Unst <- Estimates_12_Unst %>%
  filter(grepl("Alc", Column)) %>%
  filter(grepl(")",Column)) %>%
  filter(grepl("WI",Column)) 
Estimates_12_Alc_BW_Unst <- Estimates_12_Unst %>%
  filter(grepl("Alc", Column)) %>%
  filter(grepl(")",Column))%>%
  filter(grepl("BW",Column))  

hist(Estimates_12_Cog_WI_Unst$pval)
hist(Estimates_12_Cog_BW_Unst$pval)
hist(Estimates_12_Can_WI_Unst$pval)
hist(Estimates_12_Can_BW_Unst$pval)
hist(Estimates_12_Alc_WI_Unst$pval)
hist(Estimates_12_Alc_BW_Unst$pval)

Estimates_12_Tab_Unst  <- Estimates_12_Unst %>%
  dplyr::select(filename, Column, Row, 
         DisplayFull, pval) %>%
...
TableEstimates_12_Unst <- Estimates_12_Tab_Unst %>%
  filter(grepl(")",Column)) %>%
...
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
Estimates_12_Stdyx <- as_tibble(do.call("rbind", stdyx.standardized12))  %>%
  mutate(Column = ifelse(param == "COGPGS", "2a) WI Cog PGS",
                  ifelse(param == "COGPGSBW", "1a) BW Cog PGS",
...
Estimates_12_Cog_WI_Stdyx <- Estimates_12_Stdyx %>%
  filter(grepl("Cog", Column)) %>%
  filter(grepl(")",Column)) %>%
  filter(grepl("WI",Column)) 
Estimates_12_Cog_BW_Stdyx <- Estimates_12_Stdyx %>%
  filter(grepl("Cog", Column)) %>%
  filter(grepl("",Column)) %>%
  filter(grepl("BW",Column)) 
Estimates_12_Can_WI_Stdyx <- Estimates_12_Stdyx %>%
  filter(grepl("Mj", Column)) %>%
  filter(grepl(")",Column)) %>%
  filter(grepl("WI",Column)) 
Estimates_12_Can_BW_Stdyx <- Estimates_12_Stdyx %>%
  filter(grepl("Mj", Column)) %>%
  filter(grepl("",Column)) %>%
  filter(grepl("BW",Column)) 
Estimates_12_Alc_WI_Stdyx <- Estimates_12_Stdyx %>%
  filter(grepl("Alc", Column)) %>%
  filter(grepl(")",Column)) %>%
  filter(grepl("WI",Column)) 
Estimates_12_Alc_BW_Stdyx <- Estimates_12_Stdyx %>%
  filter(grepl("Alc", Column)) %>%
  filter(grepl(")",Column))%>%
  filter(grepl("BW",Column))  

hist(Estimates_12_Cog_WI_Stdyx$pval)
hist(Estimates_12_Cog_BW_Stdyx$pval)
hist(Estimates_12_Can_WI_Stdyx$pval)
hist(Estimates_12_Can_BW_Stdyx$pval)
hist(Estimates_12_Alc_WI_Stdyx$pval)
hist(Estimates_12_Alc_BW_Stdyx$pval)

Estimates_12_Tab_Stdyx  <- Estimates_12_Stdyx %>%
  dplyr::select(filename, Column, Row, 
         DisplayFull, pval) %>%
  mutate(EstP = as.numeric(pval)) %>%
  group_by(Row, Column) %>%
...
TableEstimates_12_Stdyx <- Estimates_12_Tab_Stdyx %>%
  filter(grepl(")",Column)) %>%
  spread(key = Column, value = DisplayFull) 
...
write_csv(x = TableEstimates_12_Unst,
          na = "",
          path = paste0(getwd(), "/Work/Results/", Model, "_Unst_Filt_", date,".csv"))

write_csv(x = TableEstimates_12_Stdyx,
          na = "",
          path = paste0(getwd(), "/Work/Results/", Model, "_Stdyx_Filt_", date,".csv"))
